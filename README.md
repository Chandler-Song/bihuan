...
### 核心功能
- **自然语言任务创建** - 输入「明天下午开会 #工作 #重要」，本地规则快速解析内容、优先级、提醒时间和标签，毫秒级响应
- **智能提醒升级** - 0→1次提醒+3天，1→2次+1天，≥2次标红紧急，防止任务遗忘
- **任务编辑** - 支持修改内容、优先级、备注、精确到分钟的提醒时间，编辑时自动显示已有标签
- **标签系统** - 独立标签表，支持标签复用、按标签筛选、多标签组合查询，任务列表前置显示标签
- **筛选排序** - 关键字搜索、日期范围、状态、标签筛选，支持4种排序方式
- **已闭环任务管理** - 支持查看、搜索、编辑已完成任务，可重新打开设为未完成状态，显示闭环时间
- **周/月总结** - AI 生成详细总结报告，统计闭环率、逾期数、平均闭环天数、内容分类统计
- **用户配置** - 提醒邮箱、每日提醒时间、周报/月报订阅开关
- **移动端适配** - 响应式布局优化，完美支持移动端浏览和操作
...
### 4. 已闭环任务管理
- **查看**: 在任务列表页切换到「已闭环」或「全部」状态
- **搜索**: 关键字搜索包含已闭环任务
- **编辑**: 点击已闭环任务的「编辑」按钮修改内容、标签等
- **重新打开**: 点击「重新打开」按钮将任务状态改回未完成，可继续跟踪
- **闭环时间**: 已闭环任务显示闭环时间，方便追溯
